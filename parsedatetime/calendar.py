# -*- coding: utf-8 -*-
"""
parsedatetime/calendar.py

Main class used for datetime parsing.
Uses Constants to store the regexes, Context and ContextStack to add
contextual and precision information about the parsing
"""
...
class Calendar(object):
    ...
    def parseDate(self, dateString, sourceTime=None):
        ...
        for i in range(0, 3):
            ...
            if n >= 0:
                d[c] = n
                accuracy.append({'m': Context.ACU_MONTH,
                                 'd': Context.ACU_DAY,
                                 'y': Context.ACU_YEAR}[c])
    ...
    def _evalDT(self, datetimeString, sourceTime):
        ...
        if sourceTime is None:
            sourceTime = self._parse_date_rfc822(s)
            debug and log.debug(
    ...
    def _evalDayStr(self, datetimeString, sourceTime):
        ...
        self.currentContext.updateAccuracy(Context.ACU_DAY)
        start = datetime.datetime(yr, mth, dy, startHour,
                                  startMinute, startSecond)
        target = start + datetime.timedelta(days=offset)
        return target.timetuple()
    ...
    def _evalTimeStr(self, datetimeString, sourceTime):
        ...
        if s in self.ptc.re_values['now']:
            self.currentContext.updateAccuracy(Context.ACU_NOW)
        else:
            ...
            if sTime:
                sourceTime = sTime
            self.currentContext.updateAccuracy(Context.ACU_HALFDAY)
    ...
    def _evalMeridian(self, datetimeString, sourceTime):
        ...
        if m is not None:
            ...
            if len(dt) <= 2:
                ...
                hr, mn, sec = self._extract_time(m)
    ...
    def parse(self, datetimeString, sourceTime=None, version=None):
        ...
        If the C{datetimeString} is parsed and date/time value found, then::
            ...
            will be an instance of L{Context}
    ...
    def _parse_date_rfc822(self, dateString):
        '''Parse an RFC822, RFC1123, RFC2822, or asctime-style date'''
        data = dateString.split()
        if data[0][-1] in (',', '.') or data[0].lower() in set(['mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun']):
            del data[0]
        if len(data) == 4:
            s = data[3]
            s = s.split('+', 1)
    ...
    #   these later
    def _parse_date_w3dtf(self, dateString):
        ...
        if m is None or m.group() != dateString:
            return
        return self._extract_date(m) + self._extract_time(m) + (0, 0, 0)
